...
var login = function() {
    $.post({
        ...
        success(response) {
            ...
            if (status === "Login successful") { location.reload(); } else { error("login-input"); }
        }
    });
};
...
$(document).ready(function() {
    ...
    $(document).on("click", "#signup-button", function() {
        $.post({
            ...
            success(response) {
                var status = JSON.parse(response)["status"];
                if (status === "Signup successful") { location.reload(); } else { message(status, true, "signup-box"); }
            }
        });
    });
...
// Abrir ou fechar o menu do celular e tablet
